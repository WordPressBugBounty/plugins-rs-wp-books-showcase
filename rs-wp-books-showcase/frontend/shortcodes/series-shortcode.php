/* ==========================================
File: frontend\shortcodes\series-shortcode.php
========================================== */

<?php
/**
 * Author Shortcode
 */
add_shortcode('rswpbs_series_shortcode', 'rswpbs_series_shortcode_output');
function rswpbs_series_shortcode_output($atts)
{
    $atts = shortcode_atts(
        array(
            'layout' => 'standard_layout',
            'show_description' => 'true',
            'show_book_count' => 'true',
            'series_per_row' => '4',
            'series_per_page' => '4',
        ),
        $atts
    );
    ob_start();
    ?>
    <div class="rswpbs-authors-shortcode-wrapper" data-per-row="<?php echo esc_attr($atts['series_per_row']); ?>"
        data-show-desc="<?php echo esc_attr($atts['show_description']); ?>"
        data-show-count="<?php echo esc_attr($atts['show_book_count']); ?>"
        data-per-page="<?php echo esc_attr($atts['series_per_page']); ?>">

        <div class="rswpbs-author-search">
            <form class="rswpbs-author-search-form">
                <input type="text" name="author_search" placeholder="<?php esc_attr_e('Search Series...', 'rswpbs'); ?>"
                    class="author-search-input">
                <button type="submit"><?php esc_html_e('Search', 'rswpbs'); ?></button>
            </form>
        </div>

        <div class="authors-shortcode-row rswpbs-row" id="authors-container">
            <?php echo rswpbs_get_series_html($atts, 0, ''); ?>
        </div>

        <div class="rswpbs-load-more-wrapper">
            <button id="rswpbs-load-more" class="rswpbs-load-more-btn"><?php esc_html_e('Load More', 'rswpbs'); ?></button>
        </div>
    </div>

    <script>
        jQuery(document).ready(function ($) {
            var offset = <?php echo absint($atts['series_per_page']); ?>;
            var perPage = <?php echo absint($atts['series_per_page']); ?>;
            var atts = <?php echo wp_json_encode($atts); ?>;
            var searchQuery = '';
            var ajaxNonce = '<?php echo esc_js(wp_create_nonce('rswpbs_series_nonce')); ?>';

            // Load More Handler
            $('#rswpbs-load-more').on('click', function () {
                loadAuthors(offset, searchQuery);
            });

            // Search Handler
            $('.rswpbs-author-search-form').on('submit', function (e) {
                e.preventDefault();
                searchQuery = $('.author-search-input').val();
                offset = 0;
                $('#authors-container').empty();
                loadAuthors(0, searchQuery);
            });

            function loadAuthors(offsetVal, search) {
                $.ajax({
                    url: '<?php echo esc_url(admin_url('admin-ajax.php')); ?>',
                    type: 'POST',
                    data: {
                        action: 'load_more_series',
                        offset: offsetVal,
                        search: search,
                        atts: atts,
                        nonce: ajaxNonce
                    },
                    success: function (response) {
                        if (response.success) {
                            $('#authors-container').append(response.data.html);
                            offset += perPage;

                            // Hide load more button if no more authors
                            if (!response.data.has_more) {
                                $('#rswpbs-load-more').hide();
                            } else {
                                $('#rswpbs-load-more').show();
                            }
                        }
                    },
                    error: function (xhr, status, error) {
                        console.log('AJAX Error: ' + error);
                    }
                });
            }
        });
    </script>
    <?php
    return ob_get_clean();
}

add_action('wp_ajax_load_more_series', 'rswpbs_load_more_series');
add_action('wp_ajax_nopriv_load_more_series', 'rswpbs_load_more_series');
function rswpbs_load_more_series()
{
    // Nonce verification for security
    if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'rswpbs_series_nonce')) {
        wp_send_json_error(array('message' => esc_html__('Security check failed.', 'rswpbs')));
        wp_die();
    }

    $atts = isset($_POST['atts']) && is_array($_POST['atts']) ? map_deep(wp_unslash($_POST['atts']), 'sanitize_text_field') : array();
    $offset = isset($_POST['offset']) ? intval($_POST['offset']) : 0;
    $search = isset($_POST['search']) ? sanitize_text_field(wp_unslash($_POST['search'])) : '';

    // Fallback validation for shortcode attributes
    $atts = shortcode_atts(
        array(
            'layout' => 'standard_layout',
            'show_description' => 'true',
            'show_book_count' => 'true',
            'series_per_row' => '4',
            'series_per_page' => '4',
        ),
        $atts
    );

    $html = rswpbs_get_series_html($atts, $offset, $search);

    // Get the total number of matching authors for the search
    $args = array(
        'taxonomy' => 'book-series',
        'hide_empty' => false,
        'search' => $search,
    );

    $total_authors = wp_count_terms('book-series', $args);

    // Calculate if there are more authors to load
    $per_page = intval($atts['series_per_page']);
    $has_more = ($offset + $per_page) < $total_authors;

    // Only show "Load More" button if there are more than initial per_page results
    if ($offset === 0 && $total_authors <= $per_page) {
        $has_more = false; // Hide button if total results are less than or equal to per_page
    }

    wp_send_json_success(array(
        'html' => $html,
        'has_more' => $has_more
    ));
    wp_die();
}

// Authors HTML Generator
function rswpbs_get_series_html($atts, $offset, $search = '')
{
    $authorsPerRow = isset($atts['series_per_row']) ? $atts['series_per_row'] : '4';
    $authorColumnClases = 'rswpbs-col-lg-3 rswpbs-col-md-4 author-single-column';

    if ('1' == $authorsPerRow) {
        $authorColumnClases = 'rswpbs-col-lg-12 author-single-column';
    } elseif ('2' == $authorsPerRow) {
        $authorColumnClases = 'rswpbs-col-md-6 author-single-column';
    } elseif ('3' == $authorsPerRow) {
        $authorColumnClases = 'rswpbs-col-lg-6 rswpbs-col-xl-4 rswpbs-col-md-6 author-single-column';
    } elseif ('4' == $authorsPerRow) {
        $authorColumnClases = 'rswpbs-col-lg-4 rswpbs-col-xl-3 rswpbs-col-md-6 author-single-column';
    } elseif ('6' == $authorsPerRow) {
        $authorColumnClases = 'rswpbs-col-lg-3 rswpbs-col-xl-2 rswpbs-col-md-4 author-single-column';
    }

    $args = array(
        'taxonomy' => 'book-series',
        'hide_empty' => false,
        'number' => isset($atts['series_per_page']) ? intval($atts['series_per_page']) : 4,
        'offset' => intval($offset)
    );

    if (!empty($search)) {
        $args['search'] = sanitize_text_field($search);
    }

    $bookAuthorsTerms = get_terms($args);
    $output = '';

    if (!is_wp_error($bookAuthorsTerms) && !empty($bookAuthorsTerms)) {
        foreach ($bookAuthorsTerms as $author) {
            $termLink = get_term_link($author->term_id, 'book-series');
            $authorID = 'book-series_' . $author->term_id;
            $isRswpbsPro = class_exists('Rswpbs_Pro');
            $authorImage = $isRswpbsPro ? get_term_meta($author->term_id, 'rswpbs_book_series_picture', true) : '';

            $output .= '<div class="' . esc_attr($authorColumnClases) . ' rswpbs-author-col">';
            $output .= '<div class="rswpbs-single-author-wrapper">';
            if ($isRswpbsPro && !empty($authorImage)) {
                $output .= '<div class="rswpbs-author-profile-picture-wrapper">';
                $output .= '<div class="author-profile-picture-container">';
                $output .= '<a href="' . esc_url($termLink) . '">';
                $output .= '<img src="' . esc_url($authorImage) . '" alt="' . esc_attr($author->name) . '">';
                $output .= '</a>';
                $output .= '</div>';
                if ($isRswpbsPro) {
                    $output .= '<div class="author-social-links-wrapper">';
                    if (function_exists('rswpbs_pro_book_author_social_links')) {
                        ob_start();
                        rswpbs_pro_book_author_social_links($authorID);
                        $output .= ob_get_clean();
                    }
                    $output .= '</div>';
                }
                $output .= '</div>';
            }

            if ($isRswpbsPro && empty($authorImage)) {
                ob_start();
                if (function_exists('rswpbs_pro_book_author_social_links')) {
                    rswpbs_pro_book_author_social_links($authorID);
                }
                $output .= ob_get_clean();
            }

            $output .= '<div class="author-name">';
            $output .= '<h2><a href="' . esc_url($termLink) . '">' . wp_kses_post($author->name) . '</a></h2>';
            $output .= '</div>';

            if (isset($atts['show_book_count']) && 'true' == $atts['show_book_count']) {
                $post_count = get_term($author->term_id, 'book-series')->count;
                $bookText = ($post_count !== 1) ? rswpbs_static_text_books() : rswpbs_static_text_book();
                $output .= '<div class="author-book-count">';
                $output .= '<h5>';
                $output .= sprintf(
                    '<a href="%3$s">%1$d %2$s</a>',
                    intval($post_count),
                    esc_html($bookText),
                    esc_url($termLink)
                );
                $output .= '</h5>';
                $output .= '</div>';
            }

            if (isset($atts['show_description']) && 'true' == $atts['show_description'] && !empty($author->description)) {
                $output .= '<div class="author-description">';
                $output .= '<p>' . wp_kses_post($author->description) . '</p>';
                $output .= '</div>';
            }

            $output .= '<div class="view-author-profile-button">';
            $output .= '<a href="' . esc_url($termLink) . '">' . esc_html__('View Series', 'rswpbs') . '</a>';
            $output .= '</div>';
            $output .= '</div>';
            $output .= '</div>';
        }
    }

    return $output;
}