...
function rswpbs_free_import_books_from_url($json_file_url) {
    if (empty($json_file_url) || !is_string($json_file_url)) {
        return new WP_Error('invalid_url', 'Invalid JSON file URL.');
    }
    // Fetch the JSON file
    ...
    foreach ($books as $book) {
        $result = rswpbs_import_single_book($book);
...
/**
 * Import a single book from data array.
 * 
 * @param array $book Book data.
 * @return int|WP_Error Post ID on success, WP_Error or false on failure.
 */
function rswpbs_import_single_book($book) {
    ...
    if (!empty($book['details']) && is_array($book['details'])) {
        ...
        if (!empty($details['Publisher'])) {
            ...
            // Set Book Publisher taxonomy
            if (!empty($publisherName)) {
                $term = term_exists($publisherName, 'book-publisher');
                if (!$term) {
                    $term = wp_insert_term($publisherName, 'book-publisher');
                }
                if (!is_wp_error($term) && isset($term['term_id'])) {
                    wp_set_post_terms($post_id, $term['term_id'], 'book-publisher', true);
                }
            }
        }
    }
